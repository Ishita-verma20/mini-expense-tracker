...
app.post("/api/expenses", async (req, res) => {
  try {
    ...
    const expense = await db.expense.create({ data: { amount, category, date: new Date(date), note: note || null } });
...
app.put("/api/expenses/:id", async (req, res) => {
  try {
    ...
    const { amount, category, date, note } = parsed.data;
    const expense = await db.expense.update({ where: { id }, data: { amount, category, date: new Date(date), note: note || null } });
...
app.delete("/api/budgets", async (req, res) => {
  try {
    ...
    await db.budget.deleteMany({ where: { category: String(category) } });
    return res.json({ success: true });
  } catch (error) {
...
app.get("/api/analytics/monthly", async (_req, res) => {
  try {
    ...
    const expenses = await db.expense.findMany({ where: { date: { gte: months[0].start } } });
